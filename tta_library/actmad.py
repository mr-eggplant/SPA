...
class ActMAD(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs, batch_mean = self.forward_and_get_loss(x, shift_vector)
        self._update_hist(batch_mean[-768:])
        return outputs
    
    @torch.enable_grad()  # ensure grads in possible no grad context for testing
    def forward_and_get_loss(self, images, shift_vector):
        self.model.eval()
        features = self.model.layers_cls_features_with_prompts(images)
        ...
        discrepancy_loss = self.fitness_lambda * (std_mse + mean_mse)

        cls_features = features[:, -768:]

        outputs = self.model.vit.head(cls_features)
        if self.imagenet_mask:
