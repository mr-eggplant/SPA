...
vit_tr_transforms = transforms.Compose([transforms.RandomResizedCrop(224),
									transforms.RandomHorizontalFlip(),
                                    transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.2),
									transforms.ToTensor(),
									normalize])
vit_te_transforms = transforms.Compose([transforms.Resize(256),
									transforms.CenterCrop(224),
									transforms.ToTensor(),
									normalize])
vit_te_transforms_imageC = transforms.Compose([transforms.CenterCrop(224),
									transforms.ToTensor(),
									normalize])

resnet_normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
resnet_tr_transforms = transforms.Compose([transforms.RandomResizedCrop(224),
									transforms.RandomHorizontalFlip(),
                                    transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.2),
									transforms.ToTensor(),
									resnet_normalize])
resnet_te_transforms = transforms.Compose([transforms.Resize(256),
									transforms.CenterCrop(224),
									transforms.ToTensor(),
									resnet_normalize])
resnet_te_transforms_imageC = transforms.Compose([transforms.CenterCrop(224),
									transforms.ToTensor(),
									resnet_normalize])

rotation_tr_transforms = vit_tr_transforms
rotation_te_transforms = vit_te_transforms
...
def prepare_train_dataset(args):
    ...
    trset = SelectedRotateImageFolder(traindir, vit_tr_transforms, original=True, rotation=args.rotation,
...
def prepare_test_data(args, use_transforms=True, semi=False):
    if args.model.startswith('vit'): 
        ...
        te_transforms = vit_te_transforms
        te_transforms_imageC = vit_te_transforms_imageC
    else:
        print('using resnet transforms')
        te_transforms = resnet_te_transforms
        te_transforms_imageC = resnet_te_transforms_imageC
